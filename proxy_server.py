    ...
    def create_proxy(self) -> bool:
        ...
        try:
            ...
            if api_keys:
                ...
                if not disable_auth:
                    logger.error("✗ Authentication is required but not configured.")
                    logger.error("  Set one of:")
                    logger.error("    - MCP_API_KEYS=key1:client1,key2:client2")
                    logger.error("    - MCP_API_KEYS_PATH=/path/to/api_keys.json")
                    logger.error("    - MCP_DISABLE_AUTH=true (NOT RECOMMENDED)")
                    return False
                else:
